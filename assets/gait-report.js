...
(function () {
  ...
  function renderParamBarChart(canvas, params) {
    ...
    keys.forEach(function (k, i) {
      ...
      // 实测值条
      var v = p.value;
      var vX = barX + ((v || 0) / scaleMax) * barMaxW;
      var vClampedX = Math.min(vX, barX + barMaxW);
      var color = colorMap[p.status] || '#9ca3af';
      ctx.fillStyle = color;
      ctx.fillRect(barX, y - 5, vClampedX - barX, 10);

      // 数值标签 (null = 覆盖率不足未测得, 不显示 0 误导)
      ctx.fillStyle = '#1a1a2e';
      ctx.font = 'bold 11px sans-serif';
      ctx.textAlign = 'left';
      var vTxt = (v == null) ? '未测得' : fmtNum(v, 2);
      ctx.fillText(vTxt, Math.min(vClampedX + 4, w - 50), y);
      // 步宽为透视投影近似量, 数值旁标注
      if (k === 'stepWidth' && v != null) {
        ctx.fillStyle = '#9ca3af';
        ctx.font = '9px sans-serif';
        ctx.textAlign = 'right';
        ctx.fillText('近似值(受拍摄角度影响)', w - 4, y + 13);
      }
    });
  ...
  function renderAsymmetryChart(canvas, params) {
    ...
    items.forEach(function (it, idx) {
      ...
      if (it.L != null) {
        ctx.fillText(fmtNum(it.L, 2), midX - barW / 2 - 1, h - 30 - lH - 2);
      } else {
        ctx.fillStyle = '#9ca3af';
        ctx.font = '10px sans-serif';
        ctx.fillText('未测得', midX - barW / 2 - 1, h - 32);
      }
      ...
      if (it.R != null) {
        ctx.fillStyle = '#1a1a2e';
        ctx.font = 'bold 11px sans-serif';
        ctx.fillText(fmtNum(it.R, 2), midX + barW / 2 + 1, h - 30 - rH - 2);
      } else {
        ctx.fillStyle = '#9ca3af';
        ctx.font = '10px sans-serif';
        ctx.fillText('未测得', midX + barW / 2 + 1, h - 32);
  ...
  function renderReport(data, containerEl) {
        ...
        ['stepLength','strideLength','footAngle','cadence','stance'].map(function (k) {
          ...
          if (v == null) {
            ...
            return '<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid #f0f0f0;">' +
              ...
              '<span style="font-size:11px;color:#9ca3af;">未测得(样本不足)</span>' +
            '</div>';
          }
    ...
    if (r.pelvic && !r.pelvic.error) {
      var pv = r.pelvic;
      html += '<h3 style="margin:20px 0 10px 0;font-size:16px;">🦴 骨盆与髋部运动学</h3>' +
        '<div style="background:#fff;padding:16px;border-radius:10px;margin-bottom:14px;">' +
          '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px;font-size:13px;">' +
            '<div style="text-align:center;padding:8px;background:#f8f9fa;border-radius:6px;"><div style="color:#888;font-size:11px;">骨盆垂直起伏</div><div style="font-weight:700;">' + (pv.verticalOscillation != null ? pv.verticalOscillation.toFixed(1) + ' cm' : '—') + '</div></div>' +
            '<div style="text-align:center;padding:8px;background:#f8f9fa;border-radius:6px;"><div style="color:#888;font-size:11px;">起伏节奏 CV</div><div style="font-weight:700;">' + (pv.verticalCV != null ? (pv.verticalCV * 100).toFixed(0) + '%' : '—') + '</div></div>' +
            '<div style="text-align:center;padding:8px;background:#f8f9fa;border-radius:6px;"><div style="color:#888;font-size:11px;">髋屈伸活动度</div><div style="font-weight:700;">' + (pv.hipFlexion && pv.hipFlexion.meanROM != null ? pv.hipFlexion.meanROM.toFixed(0) + '°' : '—') + '</div></div>' +
            '<div style="text-align:center;padding:8px;background:#f8f9fa;border-radius:6px;"><div style="color:#888;font-size:11px;">髋屈伸不对称</div><div style="font-weight:700;">' + (pv.hipFlexion && pv.hipFlexion.asymmetry != null ? (pv.hipFlexion.asymmetry * 100).toFixed(0) + '%' : '—') + '</div></div>' +
            '<div style="text-align:center;padding:8px;background:#f0f2f5;border-radius:6px;"><div style="color:#888;font-size:11px;">骨盆侧倾/横摆/旋转</div><div style="font-weight:600;color:#9ca3af;font-size:12px;padding-top:4px;">侧拍不可测</div></div>' +
          '</div>' +
          (pv.flags && pv.flags.length > 0 ?
            '<div style="margin-top:10px;padding:8px 12px;background:#eef2ff;border-radius:6px;border-left:3px solid #667eea;">' +
              pv.flags.map(function (f) { return '<div style="font-size:12px;color:#3730a3;line-height:1.6;">• ' + f + '</div>'; }).join('') +
            '</div>' : '') +
        '</div>';
    }
    ...
    if (bp && !bp.error) {
      ...
      var scoreColor = function (s) { return s == null ? '#9ca3af' : s >= 75 ? '#10b981' : s >= 55 ? '#f59e0b' : '#dc2626'; };
      // 雷达图 SVG
      var radarSvg = '';
      var radar = bp.radar || [];
      if (radar.length >= 3) {
        var size = 200, cx = size / 2, cy = size / 2, R = size / 2 - 32, n = radar.length;
        var pt = function (i, rr) { var ang = -Math.PI / 2 + i * 2 * Math.PI / n; return (cx + rr * Math.cos(ang)).toFixed(1) + ',' + (cy + rr * Math.sin(ang)).toFixed(1); };
        var rings = '';
        [0.34, 0.67, 1].forEach(function (f) {
          var pts = [];
          for (var i = 0; i < n; i++) pts.push(pt(i, R * f));
          rings += '<polygon points="' + pts.join(' ') + '" fill="none" stroke="#e5e7eb"/>';
        });
        var axes = '', labels = '';
        radar.forEach(function (d, i) {
          var ap = pt(i, R).split(',');
          axes += '<line x1="' + cx + '" y1="' + cy + '" x2="' + ap[0] + '" y2="' + ap[1] + '" stroke="#e5e7eb"/>';
          var lp = pt(i, R + 17).split(',');
          labels += '<text x="' + lp[0] + '" y="' + lp[1] + '" font-size="9" fill="#555" text-anchor="middle" dominant-baseline="middle">' + d.label + '</text>';
        });
        var dataPts = radar.map(function (d, i) { return pt(i, R * ((d.score || 0) / 100)); });
        radarSvg = '<svg width="' + size + '" height="' + size + '" viewBox="0 0 ' + size + ' ' + size + '" style="display:block;margin:0 auto;">' +
          rings + axes + '<polygon points="' + dataPts.join(' ') + '" fill="rgba(102,126,234,0.25)" stroke="#667eea" stroke-width="2"/>' + labels + '</svg>';
      }
      html += '<h3 style="margin:20px 0 10px 0;font-size:16px;">🧠 脑功能步态画像 (亚健康筛查)</h3>' +
        '<div style="background:#fff;padding:16px;border-radius:10px;margin-bottom:14px;">' +
          ...
          '<div style="display:flex;flex-wrap:wrap;gap:14px;align-items:flex-start;">' +
            ...
            '<div style="flex:1;min-width:240px;display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:8px;">' +
              dimKeys.map(function (k) {
                ...
                return '<div style="padding:10px;background:#f8f9fa;border-radius:6px;border-left:3px solid ' + dimColors[k] + ';">' +
                  '<div style="display:flex;justify-content:space-between;align-items:center;">' +
                    '<div style="font-size:12px;font-weight:600;">' + dimIcons[k] + ' ' + dm.label + '</div>' +
