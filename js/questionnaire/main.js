...
function finishQuiz() {
  ...
  renderResult();
  // 沙箱模式(QR 分享): 直接弹登记表单 → 填完自动同步, 不再需要手动点"保存到评估报告"按钮
  // · 隐藏"返回首页"按钮 (沙箱场景无意义)
  // · 自动派发 qnr:finished → questionnaire.html 的 inline script 弹登记表单
  // · 提交后 base64 跳到 index.html?return=questionnaire&result=... 渲染完整报告
  if (window.__qnrSandbox) {
    ...
    if (tip) tip.innerHTML = "📝 正在准备您的报告…";
    // 短暂延迟让结果页先渲染, 再派发事件触发登记表单 (避免遮挡)
    setTimeout(() => {
