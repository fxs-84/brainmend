// 《回声编织者》第一章 · demo 装配（可独立页面运行，也可嵌入主游戏选择面板）
// bootVorCh1(opts) → api；stop() 完整清理（循环/音频/DOM/订阅）
...
import { Ch1MechanicalBird } from './chapters/Ch1MechanicalBird.js';
...
export function bootVorCh1({
  ...
  const CFG = {
    ...
    token: rx.token,
    skipVas: rx.skipVas,
    segs: 20, perfectPerSeg: 3,
  };

  // --- 挂载点 ---
  const mount = document.createElement('div');
  mount.id = 'vor-ch1-root';
  ...
  const chain = new SignalChain({ winSec: Math.max(0.5, 1 / CFG.freq) });
  const evaluator = new VorQualityEvaluator({ amp: CFG.amp, freq: CFG.freq });
  const ch1 = new Ch1MechanicalBird(scene, camera);
  const metronome = new Metronome();
  const hud = new HUD(mount);
  const vas = new SymptomCheck(mount);
  const recorder = new SessionRecorder({ chapter: 1, token: CFG.token });
  ...
  function finalizeAndReport(ev) {
    const summaryExtra = {
      ...
      bestCombo: drill.stats.bestCombo, repaired: ch1.repaired,
    };
    if (ev) {
  ...
  const pace = new TrainingPaceController({
    ...
    onChange: (s) => {
      ...
      if (s === 'active') {
        ...
        metronome.stop();
        // Rest 块：准静止 1.5s 自动重校准零点（每 30 秒消一次陀螺仪漂移，玩家零操作）
        ...
        finish('训练结束',
          ...
          `修复 ${ch1.repaired}/${CFG.segs}。`, { askVas: true });
      } else {
        metronome.stop();
      }
    },
  ...
  // --- 能量：命中 +20（节拍命中 +30），完美周期再 +10；每 100 能量修复 1 单元 ---
  let energy = 0;
  function addEnergy(n, flashColor) {
    if (finished) return;
    energy += n;
    if (flashColor) ch1.flash(flashColor);
    while (energy >= 100 && ch1.repaired < CFG.segs) {
      energy -= 100;
      ch1.repairSegment();
      metronome.chime([523, 659, 784, 1047], 0.11, 0.40); // 修复：长尾大三和弦钟声
      const wp = new THREE.Vector3();
      ch1.burst(ch1.orbWorldPos(wp), 24, 1.4);
      hud.floatText(`修复 ${ch1.repaired}/${CFG.segs}`, '#9fd8ff');
    }
    if (ch1.repaired >= CFG.segs) {
      pace.stop();
      ch1.flyOut();   // 修复完成：机械鸟飞出光球，金色轨迹指向钟楼
      finish('修复完成！',
        `机械鸟右翼 ${CFG.segs} 个单元全部展开，已飞出指引钟楼！<br>` +
        `命中 ${drill.stats.hits} 次 · 节拍命中 ${drill.stats.onBeat} · 最佳连击 x${drill.stats.bestCombo}`, { askVas: true });
    }
    ch1.setEnergy(energy / 100);
  }

  // --- 靶心命中（核心玩法：摆到目标方位 = 击中，左右交替 = VOR 往复）---
  const drill = new TargetDrill({ amp: CFG.amp });
  ch1.setTargetDelta(drill.bearing);
  drill.onSpawn(({ bearing }) => ch1.setTargetDelta(bearing));
  drill.onHit(({ onBeat, combo }) => {
    const wp = new THREE.Vector3();
    ch1.burst(ch1.orbWorldPos(wp), onBeat ? 40 : 25, onBeat ? 2.0 : 1.4);
  ...
  evaluator.onTurn(() => {
    const wp = new THREE.Vector3();
    ch1.burst(ch1.orbWorldPos(wp), 6, 0.8);
  });
  ...
  async function begin(m) {
    ...
    input.startCalibration(2.0, () => {
      ...
      if (m === 'auto') input.enableAuto(CFG.amp, CFG.freq);
  ...
  if (mode) {
    ...
    hud.showOverlay('回声编织者 · 第一章（demo）',
      '佩戴头戴陀螺仪，坐在显示器前（约一臂距离），座椅固定无滑轮。<br>' +
      '左右两侧会交替出现<b>目标环</b>：转头把它<b>套进光球并保持</b>即命中，' +
      '踩着节拍命中可连击。命中积攒能量，修复机械鸟右翼（20 单元）。<br>' +
  ...
  function tick() {
    ...
    evaluator.update({ yaw: pose.yaw, wyaw: sig.omega.yaw, alphaRMS: sig.alphaRMS, valid: sig.valid && judging, t: pose.t });
    ...
    // 毛刺惩罚（4.1.4）：平滑度掉线 → 断口火花变大 + 金属摩擦声（连 3 次整体闪红）
    if (judging && ch1.setSmooth(sig.valid && evaluator.isSmooth)) metronome.grind();
    // 靶心命中：仅 Active 块判定；靶心方位 = 最新周期中线 ± amp（自动吸收陀螺仪漂移）
    const ds = judging
      ? drill.update(dt, pose.yaw, metronome.nearestBeatDelta(), evaluator.lastCycle?.midline ?? 0)
      : { err: 0, hold: 0 };
    ch1.setTargetLock(ds.hold);
    pace.update(dt);
    ch1.update(dt);

    // 固定场景模式（实测反馈：世界随头反扫不真实）——相机固定不转，场景静止；
    // 光球仍是相机子节点钉屏幕中心，只有靶环按"头相对目标的角差"滑动：
    // 头越接近目标方位，靶环越滑向中心，与光球重合即命中（判定链只看角度，不受影响）
    ch1.setTargetDelta(drill.bearing - pose.yaw);

    hud.setAngle(pose.yaw, CFG.yawLimit, CFG.amp);
    hud.setLamp(evaluator.isSmooth, sig.alphaRMS, evaluator.threshold, judging);
    hud.setProgress(ch1.repaired, CFG.segs, drill.stats.hits, drill.stats.combo);

    if (!noRender) {
      if (useComposer) composer.render(); else renderer.render(scene, camera);
    } else {
      scene.updateMatrixWorld(true);   // E2E 不渲染但保持矩阵最新（orbScreenX/propScreenX 的 project 用）
    }
  ...
  // --- E2E / 调试钩子 ---
  const api = {
    CFG, input, chain, evaluator, pace, ch1, ble, drill, stop, recorder, vas,
    orbScreenX() { const v = new THREE.Vector3(); ch1.orb.getWorldPosition(v); v.project(camera); return v.x; },
    propScreenX() { const v = new THREE.Vector3(); ch1.probeProp.getWorldPosition(v); v.project(camera); return v.x; },
    ringScreenX() { const v = new THREE.Vector3(); ch1.target.getWorldPosition(v); v.project(camera); return v.x; },
  };
