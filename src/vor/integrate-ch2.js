// 回声编织者 · 第二章《沉默的钟楼·垂直 VOR》— 接入游戏选择面板
// 模式与 src/vor/integrate.js 一致：注入模式按钮 → 拦截开始按钮 → 启动 demo
...
(function () {
  ...
  function injectButton() {
    ...
    if (document.querySelector('.mode-btn[data-mode="vorch2"]')) return true;
    ...
    btn.dataset.mode = 'vorch2';
    btn.textContent = '🕰️ 回声编织者·第二章 - 垂直VOR';
    ...
    btn.addEventListener('click', (e) => {
      ...
      if (window.gameUI) window.gameUI.selectedMode = 'vorch2';
    }, true);
    return true;
  }

  // 拦截开始按钮：vorch2 模式启动第二章 demo 而非 2D
  function hookStartBtn() {
    const startBtn = document.getElementById('start-game-btn');
    if (!startBtn || startBtn._vorCh2Hooked) return;
    startBtn._vorCh2Hooked = true;
  ...
  function hookBackBtns() {
    ['game-back-to-menu', 'back-btn-game'].forEach(id => {
      ...
      if (btn && !btn._vorCh2Hooked) {
        btn._vorCh2Hooked = true;
