// 面板集成 E2E：主游戏选择面板 → 回声编织者按钮 → 启动 demo → 陀螺仪通道 → 返回菜单
// 用法：node tests/e2e/vor-ch1-panel.spec.cjs（自起静态服务器，端口 8797）
...
(async () => {
  ...
  try {
    ...
    const btn = await page.waitForSelector('#game-select-panel .mode-btn[data-mode="vorch1"]', { timeout: 10000 });
    assert('面板中注入「回声编织者」按钮', !!btn,
      btn ? (await btn.textContent()).trim() : '未找到');

    console.log('2) 启动 demo');
    await page.evaluate(() => {
      document.querySelector('#game-select-panel .mode-btn[data-mode="vorch1"]').click();
      document.getElementById('start-game-btn').click();
    });
    await page.waitForSelector('#vor-ch1-root', { timeout: 10000 });
    assert('demo 容器挂载（#vor-ch1-root）', true);
    ...
    await page.evaluate(() => {
      const b = [...document.querySelectorAll('#vor-ch1-root #ov-btns button')].find(x => x.textContent.includes('开始训练'));
      b.click();
    });
    ...
    await page.evaluate(() => {
      ...
      window.__panelDrive = setInterval(() => {
        phase += 0.016;
        window.updateFromGyroscope({ yaw: 15 * Math.sin(2 * Math.PI * 0.5 * phase), pitch: 0, roll: 0 });
      }, 16);
    });
    await page.waitForFunction(() => Math.abs(window.__vorDemo.input.pose.yaw) > 5, null, { timeout: 8000 });
    const probe = await page.evaluate(() => ({
      yaw: window.__vorDemo.input.pose.yaw,
      cursorLeft: document.querySelector('#vor-ch1-root #hud-angle-cursor').style.left,
    }));
    assert('demo 收到主游戏通道陀螺仪数据', Math.abs(probe.yaw) > 5, `yaw=${probe.yaw.toFixed(1)}`);
    ...
    await page.click('#vor-ch1-back-btn');
    await page.waitForTimeout(500);
    assert('demo 容器已移除', await page.evaluate(() => !document.getElementById('vor-ch1-root')));
